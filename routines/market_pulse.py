...
def analyze_regime(articles: list[dict], prev: dict) -> dict:
    ...
    prompt = f"""คุณคือนักวิเคราะห์การลงทุนไทยที่เขียนบทวิเคราะห์ให้นักลงทุนรายย่อยอ่าน
สไตล์การเขียน: กระชับ เข้าใจง่าย เหมือนเพื่อนที่รู้เรื่องการเงินเล่าให้ฟัง
ห้ามแปลตรงๆ จากอังกฤษ — ให้เขียนใหม่เป็นภาษาไทยที่คนอ่านแล้วเข้าใจทันที
ศัพท์เทคนิค เช่น Fed, hawkish, risk-on, GDP คงไว้เป็นอังกฤษได้ แต่คำอธิบายรอบข้างต้องเป็นไทยธรรมชาติ

หัวข้อข่าววันนี้ (20 ชั่วโมงที่ผ่านมา):
{news_text}
...
{{
  ...
  "change_reason": "ถ้าเปลี่ยน: อธิบาย 1 ประโยคว่าเกิดอะไรขึ้น เขียนแบบเพื่อนเล่าให้ฟัง ถ้าไม่เปลี่ยนใส่ null",
  "macro_summary": "อธิบายภาพรวมตลาดวันนี้ 2 ประโยค เหมือนนักข่าวการเงินไทยเขียน ไม่ต้องแปลข่าวตรงๆ ให้สรุปว่าตลาดรู้สึกยังไงและทำไม",
  "key_drivers": ["ปัจจัย 1 เขียนเป็นภาษาไทยธรรมชาติ", "ปัจจัย 2", "ปัจจัย 3"]
}}
...
def map_portfolio_impact(regime: dict, prices: dict) -> str:
    ...
    prompt = f"""คุณคือที่ปรึกษาการลงทุนไทยที่คุยกับลูกค้าแบบตรงไปตรงมา
สไตล์: พูดเหมือนเพื่อนที่รู้เรื่องหุ้นดีมากกำลังบอกว่าวันนี้พอร์ตเราจะเป็นยังไง
ภาษาไทยธรรมชาติ อ่านแล้วเข้าใจทันที ไม่ต้องสวยหรู ไม่ต้องแปลตรงๆ จากอังกฤษ
ศัพท์เทคนิค เช่น Fed, hawkish, risk-on ใช้อังกฤษได้ แต่ต้องมีคำอธิบายไทยตามด้วยเสมอ
...
- ปัจจัยที่ขับเคลื่อนตลาด: {', '.join(regime.get('key_drivers', []))}

พอร์ตหุ้น (Ticker / กลุ่ม / ราคาล่าสุด / เปลี่ยนแปลงวันนี้ / กำไร-ขาดทุนจากต้นทุน):
{chr(10).join(rows)}

ใช้รูปแบบนี้เท่านั้น:

🟢 ได้ประโยชน์
• TICKER — อธิบายเป็นภาษาไทยธรรมชาติว่าทำไมวันนี้ถึงดี 1 บรรทัด

🔴 ได้รับผลกระทบ
• TICKER — อธิบายเป็นภาษาไทยธรรมชาติว่าทำไมวันนี้ถึงกดดัน 1 บรรทัด

⚪ ทรงตัว
• TICKER — อธิบายสั้นๆ 1 บรรทัด

💬 สรุป: 1-2 ประโยค บอกแบบตรงๆ ว่าพอร์ตนี้วันนี้ควรระวังอะไรหรือมีโอกาสอะไร"""
